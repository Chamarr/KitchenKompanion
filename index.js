...
function addStrikethroughItemsToInventory() {
    ...
    completedItems.forEach(item => {
        ...
        // Create a new inventory item with the same structure as other inventory items
        const newItem = createInventoryItem(itemName);
        inventoryList.appendChild(newItem);

        // Remove the item from the shopping list after moving it
...
document.addEventListener("click", function(event) {
    ...
    if (!Array.from(textInputs).some(input => input === document.activeElement)) {
        keyboardImage.style.display = "none"; // Hide keyboard image
    }
});
...
function createInventoryItem(name, category) {
    ...
    // Create a container for the entire item
    const itemContainer = document.createElement('div');
    itemContainer.classList.add('item-container');

    // Create name container that will be on the left
    ...
    // Create controls container that will be on the right
    ...
    // Add both containers to the item container
...
// Initialize the inventory list with hardcoded items
...
// Call the function to add default inventory items when the page loads
